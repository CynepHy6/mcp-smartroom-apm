...
- 📈 **Автоматические графики** - визуализация данных по МОС, проблемам WebRTC

### 🔒 Безопасность и надежность
- 🔐 Локальное хранение кредов, только чтение данных
- 📑 Логирование всех операций
- 🚀 Кроссплатформенная установка (Linux, macOS, Windows)
- ⚡ Высокая производительность благодаря очистке данных
...
### 3. `create_plot` - Создание графиков
Создает графики по данным из Elasticsearch с автоматическим сохранением.

**Параметры:**
- `index` - имя индекса
- `filters` - фильтры данных
- `plot_type` - тип графика: `line`, `scatter`, `bar`, `mos_timeline`, `metrics_comparison`
- `x_field` - поле для оси X (обычно `@timestamp`)
...
create_plot(
  ...
  y_field="mos",
  group_by="userRole",
  title="Качество связи (МОС) по времени"
)
```
...
```
mcp-smartroom-apm/
├── mcp_apm_server.py           # Основной MCP сервер
├── src/
│   ├── config_utils.py         # Загрузка конфигурации
│   ├── elasticsearch_client.py # Клиент Elasticsearch
│   ├── data_processing.py      # Обработка данных и алиасы
│   └── plotting.py             # Создание графиков
├── tests/                      # Тесты
├── plots/                      # Сохраненные графики
├── index.yaml                  # Конфигурация индексов
...
- **404** — индекс не найден: проверьте название в index.yaml
...
- matplotlib, pandas (для графиков)
